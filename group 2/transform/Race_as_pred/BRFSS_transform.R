...
#MyData_Variables[,c(4,5,)] <- lapply(MyData_Variables[,c(4,5)],as.numeric)
...
# colnames(MyData_Variables)
# MyData_Variables[,-c(11,12,23)] <- lapply(MyData_Variables[,-c(11,12,23)],as.factor)
...
mma.e1_weight<-mma.big(x=m,y=data.frame(y), mediator=1:ncol(m),
                pred=data.frame(pred), alpha=1, alpha1=0.05, alpha2=0.05,
                w=MyData_Variables$Final_weight)
head(mma.e1$results)
summary(mma.e1$results)
summary(mma.e1_weight)

save(MyData_Variables,
     mma.e1,
     file = "BRFSS_Transform_Race.Rdata",
     compress = TRUE)
save(MyData_Variables,
     mma.e1_weight,
